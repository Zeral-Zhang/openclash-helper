...
快速添加网站到 OpenClash 规则的浏览器插件，支持路由器同步和云端同步两种模式，并提供完整导入导出与 WebDAV 备份同步。
...
- 🚀 **路由器同步模式**: 直接通过 LuCI API 添加规则到路由器本地文件
...
- 💾 **完整导入/导出**: 一键导出完整配置与规则快照，也可从备份文件恢复
- ☁️ **WebDAV 备份同步**: 支持手动备份、从 WebDAV 恢复、双向同步
- 🔁 **WebDAV 自动同步**: 支持配置变更后自动同步，以及后台定时同步
...
- **导出完整备份**：导出本地配置、Cloudflare 配置、WebDAV 配置，以及可读取到的规则内容
- **导入备份**：从 JSON 备份恢复配置，并尽量恢复云端/路由器规则
- **备份到 WebDAV**：只填写 WebDAV 目录地址，插件会自动在其下维护 `open-clash-helper/open-clash-helper-backup.json`
- **从 WebDAV 恢复**：下载并恢复 WebDAV 中的备份
- **立即同步**：比较本地与 WebDAV 备份时间，自动选择上传或拉取
- **自动同步**：启用后，规则或配置变更会尝试自动同步，后台也会按设定间隔执行同步

> 注意：备份文件包含密码、密钥等敏感信息，请使用私有 WebDAV 并妥善保管备份文件。
